  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex flex-col items-center group cursor-pointer transition-all duration-200 hover:scale-115">
        <!-- SVG Pin Shape -->
        <svg class="w-9 h-11 drop-shadow-xl filter" viewBox="0 0 24 30" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <span class="absolute top-[6px] text-sm select-none z-10">${iconEmoji}</span>
        <!-- Active submitted radar pulse -->
        ${status === 'submitted' ? `<span class="absolute top-2 w-8 h-8 rounded-full animate-ping bg-red-500 opacity-25 pointer-events-none"></span>` : ''}
      </div>
    `,
    iconSize: [36, 44],
    iconAnchor: [18, 44],
...
const activeReportingIcon = L.divIcon({
  ...
  html: `
    <div class="relative flex flex-col items-center animate-bounce cursor-pointer">
      <svg class="w-11 h-14 drop-shadow-2xl filter" viewBox="0 0 24 30" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <span class="absolute top-[7px] text-base select-none z-10">📍</span>
    </div>
  `,
  iconSize: [44, 56],
  iconAnchor: [22, 56],
  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex flex-col items-center group cursor-pointer transition-all duration-200 hover:scale-115 ${isSelected ? 'scale-115' : ''}">
        <!-- SVG Pin Shape -->
        <svg class="w-8 h-10 drop-shadow-md filter" viewBox="0 0 24 30" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M12 0C5.37 0 0 5.37 0 12C0 19.5 12 30 12 30C12 30 24 19.5 24 12C24 5.37 18.63 0 12 0Z" fill="${color}" />
        ...
        <!-- Category Emoji Centered -->
        <span class="absolute top-[5px] text-[10px] font-sans select-none z-10">${info.emoji}</span>
        <!-- Optional pulse outer ring if selected -->
        ${isSelected ? `<span class="absolute top-1 w-6 h-6 rounded-full animate-ping bg-emerald-400 opacity-20 pointer-events-none"></span>` : ''}
      </div>
    `,
    iconSize: [32, 40],
    iconAnchor: [16, 40],
